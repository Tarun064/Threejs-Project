...
void main(){
  vec2 uv = vUv; // uv ki value default 0 t0 1 chlti hai aur uski position hoti hai left bottom edge pe
  uv -= .5;  
  ...
  uv *= distort;
  uv += .5;  // toh inn sbb mathematical operation se meine us point ko image ke bich mein shift kiya hai

  if(uv.x < 0.0 || uv.x > 1.0 || uv.y < 0.0 || uv.y > 1.0){  // aur isse image ajjeb si ho gyi kyunki copy krne lga 
    discard;     //tha last waale edges ko toh ussse hmne discard kiya hai
  }
